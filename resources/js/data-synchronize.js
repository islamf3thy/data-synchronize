$(() => {
        ...
        httpClient
            ...
            .then(async ({ headers, data }) => {
                if (headers['content-disposition'] === undefined) {
                    ...
                    BinaLinq.showError(responseJson?.message || $form.data('error-message'))
                ...
                BinaLinq.showSuccess($form.data('success-message'))
            })
            .catch((error) => {
                BinaLinq.showError(error.message ? (error.statusText + ': ' + error.message) : form.data('error-message'))
            })
    }
    ...
    if ($form.length > 0) {
        ...
        const cleanup = () => {
            BinaLinq.hideButtonLoading($button)
        ...
        dropzone.on('sending', () => {
            ...
            BinaLinq.showButtonLoading($button)
        })
